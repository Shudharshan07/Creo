...
from app.services.costume_service import generate_costume_plans

router = APIRouter(prefix="/projects/{project_id}/costumes", tags=["Costumes"])


class CostumeGenerateRequest(BaseModel):
    prompt: str
...
async def generate_costumes(
    project_id: uuid.UUID,
    body: CostumeGenerateRequest,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    project = await _get_project_or_404(project_id, current_user, db)
    result = await generate_costume_plans(body.prompt, project)
